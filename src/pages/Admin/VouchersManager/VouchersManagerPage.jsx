...
const VouchersManagerPage = () => {
  ...
  const [expirationDate, setExpirationDate] = useState(new Date().toISOString().split('T')[0]); // Set default to today's date // New state for expiration date filter
  ...
  const fetchVoucher = async () => {
    try {
      ...
      if (res && res.data && Array.isArray(res.data.rows)) {
        ...
        if (expirationDate) {
          filteredVouchers = filteredVouchers.filter(voucher => new Date(voucher.expired_date) >= new Date(expirationDate));
        }
                  ...
                  <td>
                    <div
                      className={`badge uppercase w-20 font-bold text-gray-100 ${
                        voucher.status === "active" ? "badge-success" : "badge-error"
                      }`}
                    >
                      {voucher.status}
                    </div>
