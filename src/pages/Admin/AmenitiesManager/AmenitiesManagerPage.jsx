...
const AmenitiesManagerPage = () => {
    ...
    const [amenities, setAmenities] = useState([
      { id: "AM01", name: "Amenity A", images: [], originalPrice: 100000, depreciationPrice: 80000, rentPrice: 20000, type: "type1", status: "Active" },
      { id: "AM02", name: "Amenity B", images: [], originalPrice: 200000, depreciationPrice: 90000, rentPrice: 30000, type: "type2", status: "Inactive" },
      { id: "AM03", name: "Amenity C", images: [], originalPrice: 300000, depreciationPrice: 100000, rentPrice: 40000, type: "type3", status: "Active" },
    ]);
    const [searchTerm, setSearchTerm] = useState("");
    const [showAddModal, setShowAddModal] = useState(false);
    const [showUpdateModal, setShowUpdateModal] = useState(false);
    const [showDeleteModal, setShowDeleteModal] = useState(false);
    const [currentAmenity, setCurrentAmenity] = useState({ id: "", name: "", images: "", originalPrice: "", depreciationPrice: "", rentPrice: "", type: "", status: "" });
  ...
  const updateAmenityFields = [
    ...
    const generateAmenityId = () => {
        const lastId = amenities.length > 0 ? amenities[amenities.length - 1].id : "AM00";
        const newId = `AM${(parseInt(lastId.substring(2)) + 1).toString().padStart(2, '0')}`;
        return newId;
    };

    const handleAddAmenitySubmit = (e) => {
        e.preventDefault();
        const newAmenity = { ...currentAmenity, id: generateAmenityId(), status: "Active" };
        setAmenities([...amenities, newAmenity]);
        setShowAddModal(false);
        setSuccessMessage("Amenity Added Successfully!");
        setCurrentAmenity({ id: '', name: '', images: '', originalPrice: '', depreciationPrice: '', rentPrice: '', type: '', status: '' });
    };

    const handleUpdateAmenitySubmit = (e) => {
      e.preventDefault();
      setAmenities((prevAmenities) => {
          const amenityIndex = prevAmenities.findIndex((amenity) => amenity.id === currentAmenity.oldId);
          if (amenityIndex !== -1) {
