...
const BookingsManagerPage = () => {
 ...
 // Open modal truyền vào 1 booking để hiển thị chi tiết booking đó
  const handleOpenModal = (selectedBooking) => { // param: selectedBooking là booking được chọn để hiển thị chi tiết
        ...
        <div className="stats shadow">
          ...
          <div className="stat place-items-center">
            ...
            <div className="stat-value text-success">
              {bookings.filter(b => b.BookingStatuses[0]?.status === 'usage').length}
            </div>
          </div>
        </div>
      </div>
                ...
                <tr key={booking.booking_id}>
                  ...
                  <td>
                    {booking.BookingStatuses.length > 0 ? (
                      ...
                      <div className="badge badge-neutral text-lg mx-1 my-1 rounded-lg shadow-md">N/A</div>
                    )}
                  </td>
                  <td>{booking.total_price}</td>
                  <td>
                    <button className="btn btn-sm hover:bg-green-500" onClick={() => handleOpenModal(booking)}>Details</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {openModal && selectedBooking && (
        <dialog id="my_modal_3" className="modal open" open>
          <div className="modal-box">
            ...
            <p className="py-4">Building Name: {selectedBooking.Workspace.Building.building_name}</p>
            <p className="py-4">Workspace Name: {selectedBooking.Workspace.workspace_name}</p>
            <p className="py-4">Booking Type: {selectedBooking.BookingType.type}</p>
            <p className="py-4">Start Time: {selectedBooking.start_time_date}</p>
            <p className="py-4">End Time: {selectedBooking.end_time_date}</p>
            <p className="py-4">Status: {selectedBooking.BookingStatuses[0]?.status || "N/A"}</p>
