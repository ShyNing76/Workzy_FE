...
const StaffsManagerPage = () => {
    ...
    const [currentStaff, setCurrentStaff] = useState({ id: "", fname: "", lname: "", info: "" });
    ...
    const generateStaffId = () => {
        ...
        const newId = `ST${(parseInt(lastId.substring(2)) + 1).toString().padStart(2, '0')}`;
        return newId;
    };

    const handleAddStaffSubmit = (e) => {
        e.preventDefault();
        const newStaff = { ...currentStaff, id: generateStaffId(), name: `${currentStaff.fname} ${currentStaff.lname}` };
    ...
    const handleUpdateStaffSubmit = (e) => {
        ...
        setStaffs((prevStaffs) => {
            const staffIndex = prevStaffs.findIndex((staff) => staff.id === currentStaff.oldId);
            if (staffIndex !== -1) {
                const updatedStaffs = [...prevStaffs];
                updatedStaffs[staffIndex] = { ...currentStaff, name: `${currentStaff.fname} ${currentStaff.lname}` };
                  ...
                  filteredStaffs.map((staff) => (
                    <tr key={staff.id}>
                      ...
                      <td>
                        ...
                        <DeleteButton
                          onClick={() => {
                            setStaffToDelete({...staff, name: `${staff.lname} ${staff.fname}`,});
