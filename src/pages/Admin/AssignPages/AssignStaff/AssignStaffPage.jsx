...
import { getStaff, getBuilding, unassignStaffFromBuilding, assignStaffToBuilding } from '../../../../config/api.admin'
...
const AssignStaffPage = () => {
    ...
    // Selected staff id for each building
    ...
    const fetchStaffAndBuilding = async () => {
        setIsLoading(true);
        try {
            const [staffResponse, buildingResponse] = await Promise.all([
                getStaff(),
                getBuilding(),
            ])
            if (staffResponse && staffResponse.data && staffResponse.err === 0) {
                const activeStaff = staffResponse.data.rows.filter((staff) => staff.status === "active")
  ...
  const searchBuildings = searchTerm.trim()
    ...
    const handleSelectedStaffId = (userId, buildingId) => {
        ...
        if (isStaffAssigned(userId, buildingId)) { // nếu nhân viên đã được phân công cho tòa nhà hiện tại
            setSelectedStaffIds((prevSelectedStaffIds) => ({ // cập nhật lại selectedStaffIds với staffId mới
        ...
        } else { // nếu tòa nhà không được phân công cho nhân viên nào
            setSelectedStaffIds((prevSelectedStaffIds) => ({ // cập nhật lại selectedStaffIds với staffId mới
                ...prevSelectedStaffIds,
                [buildingId]: userId,
            }));
