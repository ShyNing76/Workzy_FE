...
const WorkspacesTypesManagerPage = () => {
    ...
    const [workspaceType, setWorkspaceType] = useState([
        { id: "TY1", name: "Type 1", image: [], description: "Loai 1", status: "Active" },
        { id: "TY2", name: "Type 2", image: [], description: "Loai 2", status: "Inactive" },
        { id: "TY3", name: "Type 3", image: [], description: "Loai 3", status: "Active"}
    ]);
    ...
    const generateWorkspaceTypeId = () => {
        const lastId = workspaceType.length > 0 ? workspaceType[workspaceType.length - 1].id : "TY0";
    ...
    const handleAddWorkspaceType = (e) => {
        e.preventDefault();
        const newWorkspace = { ...currentWorkspaceType, id: generateWorkspaceTypeId(), status: "Active" };
        setWorkspaceType([...workspaceType, newWorkspace]);
        setShowAddModal(false);
        setSuccessMessage('Workspace type added successfully!');
        setCurrentWorkspaceType({ id: "", name: "", image: [], description: "", status: "" });
    }

    const handleUpdateWorkspaceType = (e) => {
        e.preventDefault();
        setWorkspaceType((prevWorkspaceTypes) => {
            const workspaceTypeIndex = prevWorkspaceTypes.findIndex((workspaceType) => workspaceType.id === currentWorkspaceType.oldId);
    ...
    const handleDeleteWorkspaceType = () => {
        setWorkspaceType((prevWorkspaceTypes) => 
            prevWorkspaceTypes.filter((workspaceType) => workspaceType.id !== workspaceTypeToDelete.id)
        );
                ...
                filteredWorkspaceType.map((workspaceType) => (
                  <tr key={workspaceType.id}>
                    ...
                    <td>
                      ...
                      <UpdateButton
                        onClick={() => {
                          setCurrentWorkspaceType({ ...workspaceType, oldId: workspaceType.id });
