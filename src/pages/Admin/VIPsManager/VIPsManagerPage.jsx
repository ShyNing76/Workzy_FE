...
const VIPsManagerPage = () => {
    ...
    const [currentVIP, setCurrentVIP] = useState({ id: "", fname: "", lname: "", type: "" });
    ...
    const generateVIPId = () => {
        ...
        const newId = `V${(parseInt(lastId.substring(1)) + 1).toString().padStart(2, '0')}`;
        return newId;
    };

    const handleAddVIPSubmit = (e) => {
        e.preventDefault();
        const newVIP = { ...currentVIP, id: generateVIPId(), name: `${currentVIP.fname} ${currentVIP.lname}` };
        setVIPs([...VIPs, newVIP]);
    ...
    const handleUpdateVIPSubmit = (e) => {
        ...
        setVIPs((prevVIPs) => {
            ...
            if (VIPIndex !== -1) {
                ...
                updatedVIPs[VIPIndex] = { ...currentVIP, name: `${currentVIP.fname} ${currentVIP.lname}` };
                return updatedVIPs;
            }
            return prevVIPs;
        });
    ...
    const handleDeleteVIP = () => {
        setVIPs((prevVIPs) =>
            prevVIPs.filter((VIP) => VIP.id !== VIPToDelete.id)
        );
                  ...
                  filteredVIPs.map((VIP) => (
                    <tr key={VIP.id}>
                      ...
                      <td>
                        ...
                        <DeleteButton
                          onClick={() => {
                            setVIPToDelete({...VIP, name: `${VIP.lname} ${VIP.fname}`,});
